    ...
    const _getAllPaginatedResources = (responseHeaders) => {
        ...
        if(responseHeaders.link) {
            ...
            for (let i = 0; i < pageCount; i++) {
                let pageUri = links.last.replace(/\d+$/g, `${i + 1}`)
                pages.push(pageUri)
            }
        }
        return pages
        ...
        return co(function* () {
            ...
            if (paginatedResources.length > 1) {
                ...
                return Promise.all(promises).then(function (results) {
                    ...
                    return co(function* () {
                        const filledResults = yield _fillRawContent(allResults)
                        return _filter(filledResults, filterBy)
                    })
                })
            }
            const filledResults = yield _fillRawContent(page1Response.data)
            return _filter(filledResults, filterBy)
        })
    }
